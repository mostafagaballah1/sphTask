package com.mostafa.assignment.data.mapper
    ...
    fun mapToDomainErrorException(throwable: Throwable?): ErrorModel {
        val errorModel: ErrorModel? = when (throwable) {
            ...
            is UnknownHostException -> {
                ErrorModel("Un known host exception, data loaded from cache if any",0,ErrorStatus.EMPTY_RESPONSE)
            }

            // handle connection error
            is IOException -> {
                ErrorModel("No Internet Connection, data loaded from cache if any",0,ErrorStatus.NO_CONNECTION)
            }
